<script>
    ...
    let description = ''
    function saveList () {
        createListService(title, description)
        .then((value) => {
            if (value) {
                modalCreateListVisible.update(() => false)
            }
        })
        .catch()
        .finally()
...
<div class="modal">
    <div class="modal-content">
        <span class="modal-name">Создание списка</span>
        <div class="modal-form">
            ...
            <button 
                id="cancel"
                on:click={() => modalCreateListVisible.update(() => false)}
            >
                Отмена
            </button>
        </div>
